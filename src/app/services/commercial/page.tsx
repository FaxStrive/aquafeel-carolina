...
export default function CommercialPage() {
  ...
  const industryIcons = [UtensilsCrossed, Building2, Stethoscope, Hotel, Factory, GraduationCap];
  const industries = [
    { icon: industryIcons[0], title: p.industry1Title, description: p.industry1Desc },
    { icon: industryIcons[1], title: p.industry2Title, description: p.industry2Desc },
    { icon: industryIcons[2], title: p.industry3Title, description: p.industry3Desc },
    { icon: industryIcons[3], title: p.industry4Title, description: p.industry4Desc },
    { icon: industryIcons[4], title: p.industry5Title, description: p.industry5Desc },
    { icon: industryIcons[5], title: p.industry6Title, description: p.industry6Desc },
  ];

  const benefitIcons = [Settings2, Clock, FileCheck2, Wrench, PiggyBank, ShieldCheck];
  const benefits = [
  ...
  const stepIcons = [ClipboardList, FlaskConical, Ruler, CheckCircle2];
  const steps = [
    ...
    { number: "04", icon: stepIcons[3], title: p.step4Title, description: p.step4Desc },
  ];
      ...
      <main className="overflow-hidden">
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 z-[5]">
            <svg viewBox="0 0 1440 80" preserveAspectRatio="none" className="w-full h-16 md:h-20">
              ...
              <path d="M0,50 C360,80 720,20 1080,50 C1260,65 1380,35 1440,50 L1440,80 L0,80 Z" fill="white" opacity="0.5" />
            </svg>
          </div>
        </section>
        ...
        <section className="relative py-20 md:py-28">
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 80% 60% at 20% 30%, rgba(6,182,212,0.04) 0%, transparent 70%), radial-gradient(ellipse 60% 50% at 80% 70%, rgba(37,39,114,0.03) 0%, transparent 70%)" }} />
          </div>

          <div className="relative max-w-6xl mx-auto px-6">
            <AnimatedSection className="text-center mb-16">
              <motion.span variants={fadeUp} transition={{ duration: 0.5 }} className="inline-block mb-4 px-4 py-1.5 rounded-full bg-[var(--color-surface-alt)] text-[var(--color-secondary)] font-heading text-sm font-semibold tracking-wide uppercase">
                {p.industriesLabel}
              </motion.span>
              <motion.h2 variants={fadeUp} transition={{ duration: 0.6 }} className="font-heading text-[var(--color-text-primary)]" style={{ fontSize: "var(--text-h2)" }}>
                {p.industriesHeadline}
              </motion.h2>
              <motion.p variants={fadeUp} transition={{ duration: 0.5 }} className="mt-4 max-w-2xl mx-auto font-body text-[var(--color-text-secondary)] text-lg leading-relaxed">
                {p.industriesDescription}
              </motion.p>
            </AnimatedSection>
              ...
              {industries.map((industry, i) => (
                ...
                >
                  <div className="w-12 h-12 rounded-xl bg-[var(--color-surface-alt)] flex items-center justify-center mb-5 group-hover:bg-brand-cyan/10 transition-colors">
                    <industry.icon className="w-6 h-6 text-[var(--color-secondary)]" />
                  </div>
                  <h3 className="font-heading text-lg font-semibold text-[var(--color-text-primary)]">{industry.title}</h3>
                  <p className="mt-2.5 font-body text-[var(--color-text-secondary)] leading-relaxed">{industry.description}</p>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <section className="relative py-20 md:py-28">
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute inset-0" style={{ background: "linear-gradient(160deg, rgba(238,242,255,0.7) 0%, rgba(248,250,255,0.4) 50%, rgba(6,182,212,0.03) 100%)" }} />
          ...
          <div className="relative max-w-6xl mx-auto px-6">
            <AnimatedSection className="text-center mb-16">
              ...
              <motion.p variants={fadeUp} transition={{ duration: 0.5 }} className="mt-4 max-w-2xl mx-auto font-body text-[var(--color-text-secondary)] text-lg">
                {p.whyDescription}
              </motion.p>
            </AnimatedSection>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {benefits.map((benefit, i) => (
                ...
                >
                  ...
                  <h3 className="font-heading text-lg font-semibold text-[var(--color-text-primary)]">{benefit.title}</h3>
                  <p className="mt-2.5 font-body text-[var(--color-text-secondary)] leading-relaxed">{benefit.description}</p>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <section className="relative py-20 md:py-28">
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, var(--color-surface) 0%, rgba(248,250,255,0.5) 100%)" }} />
            <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[var(--color-border)] to-transparent" />
          ...
          <div className="relative max-w-6xl mx-auto px-6">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              {steps.map((step, i) => (
                ...
                >
                  {i < steps.length - 1 && (
                    <div className="hidden lg:block absolute top-12 left-[calc(50%+40px)] right-[calc(-50%+40px)] h-px bg-gradient-to-r from-[var(--color-border)] to-transparent" />
                  )}
                  <div className="relative p-8 rounded-2xl border border-[var(--color-border)] bg-white hover:shadow-lg hover:shadow-brand-cyan/5 transition-all duration-300">
                    <div className="flex items-center gap-4 mb-5">
                      <span className="font-heading text-4xl font-bold text-brand-cyan/20">{step.number}</span>
                    ...
                    <h3 className="font-heading text-xl font-semibold text-[var(--color-text-primary)]">{step.title}</h3>
                    <p className="mt-3 font-body text-[var(--color-text-secondary)] leading-relaxed">{step.description}</p>
                  </div>
                </motion.div>
              ))}
            </div>
        ...
        <section ref={imageRef} className="relative py-20 md:py-28">
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 70% 50% at 50% 50%, rgba(6,182,212,0.03) 0%, transparent 70%)" }} />
          </div>
          ...
          <div className="relative max-w-5xl mx-auto px-6">
            <AnimatedSection className="text-center mb-12">
              ...
              <motion.p variants={fadeUp} transition={{ duration: 0.5 }} className="mt-4 max-w-xl mx-auto font-body text-[var(--color-text-secondary)] text-lg">
        ...
        <section className="relative py-20 md:py-28">
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 70% 60% at 70% 40%, rgba(37,39,114,0.03) 0%, transparent 70%), radial-gradient(ellipse 50% 40% at 20% 80%, rgba(6,182,212,0.03) 0%, transparent 70%)" }} />
            <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[var(--color-border)] to-transparent" />
          </div>
        ...
        <section className="relative py-20 md:py-28 overflow-hidden">
          ...
          <div className="relative z-10 max-w-3xl mx-auto px-6 text-center">
            <AnimatedSection>
              <motion.h2 variants={fadeUp} transition={{ duration: 0.6 }} className="font-heading text-white" style={{ fontSize: "var(--text-h2)" }}>
                {p.ctaHeadline1}
                <span className="text-brand-cyan">{p.ctaHeadlineHighlight}</span>
              </motion.h2>
              <motion.p variants={fadeUp} transition={{ duration: 0.5 }} className="mt-5 text-lg text-white/70 font-body leading-relaxed">
                {p.ctaDescription}
              </motion.p>
              <motion.div variants={fadeUp} transition={{ duration: 0.5 }} className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
                <Link
                  href="/contact"
